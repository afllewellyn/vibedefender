...
BEGIN
    ...
    -- Show evidence type breakdown after conversion (simplified approach)
    RAISE NOTICE 'EVIDENCE TYPE BREAKDOWN:';
    
    -- Count NULL evidence
    SELECT COUNT(*) INTO type_count FROM scan_findings WHERE evidence IS NULL;
    IF type_count > 0 THEN
        RAISE NOTICE 'NULL_evidence: % rows', type_count;
    END IF;
    
    -- Count failed conversions
    SELECT COUNT(*) INTO type_count FROM scan_findings WHERE evidence IS NOT NULL AND evidence_jsonb IS NULL;
    IF type_count > 0 THEN
        RAISE NOTICE 'FAILED_conversion: % rows', type_count;
    END IF;
    
    -- Count cookies
    SELECT COUNT(*) INTO type_count FROM scan_findings WHERE evidence_jsonb->>'category' = 'cookies';
    IF type_count > 0 THEN
        RAISE NOTICE 'cookies: % rows', type_count;
    END IF;
    
    -- Count headers
    SELECT COUNT(*) INTO type_count FROM scan_findings WHERE evidence_jsonb->>'category' = 'headers';
    IF type_count > 0 THEN
        RAISE NOTICE 'headers: % rows', type_count;
    END IF;
    
    -- Count URLs/files
    SELECT COUNT(*) INTO type_count FROM scan_findings WHERE evidence_jsonb->>'category' = 'urls_files';
    IF type_count > 0 THEN
        RAISE NOTICE 'urls_files: % rows', type_count;
    END IF;
    
    -- Count raw text
    SELECT COUNT(*) INTO type_count FROM scan_findings WHERE evidence_jsonb->>'category' = 'raw_text';
    IF type_count > 0 THEN
        RAISE NOTICE 'raw_text: % rows', type_count;
    END IF;
